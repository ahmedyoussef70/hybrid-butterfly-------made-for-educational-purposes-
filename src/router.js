...
import { Component } from './hybrid-butterfly.js'
import GLOBALS from './globals.js'

GLOBALS.history.listen(() => {
	GLOBALS.appInstance.updateView()
})

export class RouterView {
	constructor({ path, component, exact, data }) {
		let browserPathname = GLOBALS.history.location.pathname
		let keys = []
...
export class RouterLink extends Component {
	...
	navigate() {
		GLOBALS.history.push(this.path)
	}
	template() {
